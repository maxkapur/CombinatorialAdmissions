...
w(\mathcal{Y}) = \sum_{j \in \{0\} \cup \mathcal{Y}} \bar t_j p_j(\mathcal{Y}) =  \sum_{j \in \{0\} \cup \mathcal{Y}} \tilde t_j p_j(\mathcal{Y})- t_0 = v(\mathcal{Y}\cup\{k\}) - t_0 \end{equation}
...
\KwData{Utility values $t \in[0, \infty)^m$, admissions probabilities $f \in [0, 1]^m$, application limit $h \leq m$.}
Index schools in ascending order by $t$\;
$\mathtt{X} \gets $ an empty list\;
$\mathtt{H} \gets \operatorname{MaxFibHeap}()$, an empty max heap of named tuples $\mathtt{C}$ ordered by $\mathtt{C.f}\cdot\mathtt{C.t}$\;
\For{$j=1\dots m$}
{
...
\end{proof}
In fact, the time complexity remains $O(h m + m\log m)$ if an array is used instead of a Fibonacci heap. We found a simple binary heap to be the most effective data structure in our numerical experiments due to its low overhead cost.
