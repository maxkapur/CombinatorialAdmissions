...
%When Algorithm \ref{algorithmforhetg} is applied to Alma's problem, with each $g_j = 1$ and $H =h$, it achieves the same time complexity as Algorithm \ref{algorithmforlargeh}. However, Algorithm \ref{algorithmforlargeh} is more effective because its space requirements are $O(m)$. 
