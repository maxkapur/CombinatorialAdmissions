...
(Inductive step.) Assume that $\mathcal{X}_1 \subset \cdots \subset \mathcal{X}_h$, and we will show that  $\mathcal{X}_h \subset  \mathcal{X}_{h+1}$. Applying the inductive hypothesis, we write $\mathcal{X}_h = \mathcal{X}_{h-1} \cup \{j\}$. To get a contradiction, suppose that $\mathcal{X}_{h+1} = \mathcal{Y}_{h-1} \cup \{k, l\}$, where $k, l \in \mathcal{X}_{h+1} \setminus \mathcal{X}_h$. We will show that if such a counterexample exists, then a counterexample exists for a market in which all schools have the same admissions probability $f_j = 1/2$. But the optimal portfolios for this facile market are clearly nested, a contradiction.

Without loss of generality, we may assume that $\mathcal{C} = \mathcal{X}_{h+1} \cup \mathcal{X}_h$. Let $\mathcal{X}_{h+1} = \{(1), \dots, (h+1)\}$ where $t_{(1)} \leq \cdots \leq t_{(h+1)}$. 

$\mathcal{X}_h$ and $\mathcal{X}_{h+1}$ remain optimal in the modified market if the following sets of inequalities are satisfied:
\begin{align} 
\forall i \in \mathcal{X}_{h+1} , \qquad w(\mathcal{X}_{h+1} \setminus i)& < w(\mathcal{X}_h)\\
\iff \label{linearsystemfortransformingtocommonf}
\begin{bmatrix}
0 & 1/2^{h} & 1/2^{h-1} & \cdots & 1/2^{2} & 1/2 \\
1/2^{h}  & 0 & 1/2^{h-1} & \cdots & 1/2^{2} & 1/2 \\
1/2^{h} & 1/2^{h-1} & 0&\cdots & 1/2^{2} & 1/2 \\
\vdots & \vdots & \vdots & \ddots & \vdots &\vdots\\
1/2^{h} & 1/2^{h-1} & 1/2^{h-2} &\cdots & 0& 1/2 \\
1/2^{h} & 1/2^{h-1} & 1/2^{h-2} &\cdots & 1/2^{2} &0\\
\end{bmatrix}
\begin{bmatrix}
t_{(1)} \\
t_{(2)} \\
...
&<
\begin{bmatrix}
%w(\mathcal{X}_h) - \varepsilon_1 \\
%w(\mathcal{X}_h) - \varepsilon_2 \\
%w(\mathcal{X}_h)- \varepsilon_3 \\
%\vdots \\
%w(\mathcal{X}_h) - \varepsilon_h \\
%w(\mathcal{X}_h) - \varepsilon_{h+1} \\
w(\mathcal{X}_h) \\
w(\mathcal{X}_h) \\
w(\mathcal{X}_h) \\
\vdots\\
...
and
\begin{align} 
\label{linearsystemfortransformingtocommonf2}
\forall i \in \mathcal{X}_{h+1} , \qquad w(\mathcal{X}_{h+1}) = \sum_{l=1}^{h+1} t_{(l)} 1/2^l  > w(\mathcal{X}_h\cup\{i\})
\end{align}
The $i$th row of this system says that $w(\mathcal{X}_{h+1} \setminus (i)) \leq v_h - \epsilon_i$, where $\epsilon_i = v_h - w(\mathcal{X}_{h+1} \setminus (i)) >0$. We will now modify the $t_j$ values so that $w(\mathcal{X}_h) = v(\mathcal{X}_h)$ and \eqref{linearsystemfortransformingtocommonf} holds with equality.

To ensure that $\mathcal{X}_h$ is still optimal at $h$ in 
...
%(Inductive step)
%Assume that $\mathcal{X}_1 \subset \cdots \subset \mathcal{X}_h$, and we will show that  $\mathcal{X}_h \subset  \mathcal{X}_{h+1}$. Pick $j \in \mathcal{X}_{h+1} \setminus \mathcal{X}_h$ and let $\mathcal{Y}_h = \mathcal{X}_{h+1} \setminus j$.
