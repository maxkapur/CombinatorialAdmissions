...
(Inductive step.) Assume that $\mathcal{X}_1 \subset \cdots \subset \mathcal{X}_h$, and we will show that  $\mathcal{X}_h \subset  \mathcal{X}_{h+1}$. Pick $j \in \mathcal{X}_{h+1} \setminus \mathcal{X}_h$ and let $\mathcal{Y}_h = \mathcal{X}_{h+1} \setminus j$.
Also, for each portfolio $\mathcal{X}$, let the random variable $X = \max\{t_j Z_j : j \in \mathcal{X}\}$ denote the utility achieved by the portfolio, so that $v(\mathcal{X}) = \operatorname{E}[X]$. 

If the theorem is false, then it must be the case that both $v(\mathcal{X}_h) > v(\mathcal{Y}_h)$ and $v(\mathcal{X}_{h+1}) > v(\mathcal{X}_h \cup\{j\})$. That is,
\begin{align}
&&v(\mathcal{X}_{h+1}) = v(\mathcal{Y}_h \cup\{j\}) &> v(\mathcal{X}_h \cup\{j\}) \\
\iff \quad&& (1 - f_j) v(\mathcal{Y}_h) + f_j \operatorname{E}\bigl[ \max\{Y_h, t_j \} \bigr] &> (1 - f_j) v(\mathcal{X}_h) + f_j \operatorname{E}\bigl[ \max\{X_h, t_j \} \bigr] \\
&&&\geq (1 - f_j) v(\mathcal{Y}_h) + f_j \operatorname{E}\bigl[ \max\{X_h, t_j \} \bigr] \\
\implies \quad && \operatorname{E}\bigl[ \max\{Y_h, t_j \} \bigr] &>  \operatorname{E}\bigl[ \max\{X_h, t_j \} \bigr]\\
\implies \quad && \operatorname{E}\bigl[ \max\{Y_h - t_j, 0 \} \bigr] &>  \operatorname{E}\bigl[ \max\{X_h- t_j, 0 \} \bigr]
\end{align}
On the other hand, 
\begin{align}
&&\Delta v(\mathcal{Y}_h, j) &> \Delta v(\mathcal{X}_h, j) \\
\implies \quad && \operatorname{E}\bigl[ \max\{0, t_j - Y_h \} \bigr] &>  \operatorname{E}\bigl[ \max\{0, t_j - X_h \} \bigr]
\end{align}
(WIP.)  
\end{proof}
...
%
%This says that if Alma is guaranteed admission to $c_j$ and allowed to choose $h$ additional schools to apply to, she would do better by choosing $\mathcal{Y}_h$ than $\mathcal{X}_h$. 
%If $t_k > t_j$ for all $k \in \mathcal{Y}_h$, then the value of $\max\{Y_h, t_j \} $ differs from that of $Y_h$ only when Alma is rejected from all schools in $Y_h$:
%\begin{equation} \operatorname{E}\bigl[ \max\{Y_h, t_j \} \bigr] = v(\mathcal{Y}_h) + t_j \prod_{k \in \mathcal{Y}_h} (1 -f_k)
%\end{equation}
%Meanwhile, since being guaranteed admission to $t_k$ improves the value of $X_h$ in this case,
%\begin{equation} \operatorname{E}\bigl[ \max\{X_h, t_j \} \bigr] \geq v(\mathcal{X}_h) + t_j \prod_{k \in \mathcal{X}_h} (1 -f_k)
%\end{equation}
%and concenating these equations yields $v(\mathcal{Y}_h) > v(\mathcal{X}_h)$, a contradiction. 
%
%On the other hand, if there is a school $k$ in $\mathcal{Y}_h$ having $t_k \leq t_j$, then removing this school from $\mathcal{Y}_h$ does not change the expected value of $ \max\{Y_h, t_j \} $. Therefore
%\begin{equation}
%\operatorname{E}\bigl[ \max\{Y_h^{\setminus k}, t_j \} \bigr] >  \operatorname{E}\bigl[ \max\{X_h, t_j \} \bigr] \geq \operatorname{E}\bigl[ \max\{X_{h-1}, t_j \} \bigr]
%\end{equation}
%By repeating this logic, we must eventually encounter a subportfolio of $Y_h$ consisting \emph{entirely} of schools for which $t_k > t_j$, yielding the contradiction above. 

%\begin{align}
%&&\operatorname{E}\bigl[ \max\{Y_h^{\setminus k}, t_j \} \bigr] &>  \operatorname{E}\bigl[ \max\{X_h, t_j \} \bigr] \\
%\implies \quad&&  (1 - f_j) v(\mathcal{X}_{h-1})  + f_j \operatorname{E}\bigl[ \max\{Y_h^{\setminus k}, t_j \} \bigr]
%&> (1 - f_j)  v(\mathcal{X}_{h-1}) + f_j  \operatorname{E}\bigl[ \max\{X_h, t_j \} \bigr] \\
%&&&\geq v(\mathcal{X}_{h-1}) + f_j  \operatorname{E}\bigl[ \max\{X_h, t_j \} \bigr] \\
%\end{align}
...
%\begin{equation} \label{xjiexpanded}
%v(\mathcal{X}_{j}^{(i)}) = (1 - f_i) v(\mathcal{X}_{j}^{(i)}\setminus i) + f_i t_i
%\end{equation}
%Optimality now requires that $\mathcal{X}_{j}^{(i)}\setminus i = \mathcal{X}_{j-1}^{(i-1)}$. If $\mathcal{X}_{j-1}^{(i)}$ does not contain $i$, then $\mathcal{X}_{j-1}^{(i-1)}  = \mathcal{X}_{j-1}^{(i)}$ and we are done. If $\mathcal{X}_{j-1}^{(i)}$ contains $i$, then expanding the expression for $\mathcal{X}_{j-1}^{(i)}$ in the manner of \eqref{xjiexpanded} implies $\mathcal{X}_{j-1}^{(i)} =  \mathcal{X}_{j-2}^{(i-1)}\cup\{i\}$. Since $i \in \mathcal{X}_{j}^{(i)}$ and $\mathcal{X}_{j-2}^{(i-1)} \subset  \mathcal{X}_{j-1}^{(i-1)}$ by the inductive hypothesis, the statement follows. 
%
%If $\mathcal{X}_{j}^{(i)}$ does not contain $i$, then $\mathcal{X}_{j}^{(i)} = \mathcal{X}_{j}^{(i-1)}$ since the former is optimal over a larger set of schools. It follows by \eqref{himinusonenested} that $\mathcal{X}_{j-1}^{(i-1)} \subset \mathcal{X}_{j}^{(i)}$. %Now, if $\mathcal{X}_{j-1}^{(i-1)} $ does not contain $i$, we are done; if not, the argument of the previous paragraph implies that $\mathcal{X}_{j-2}^{(i-1)} \subset  \mathcal{X}_{j-1}^{(i-1)}$. 
...
%
%For the inductive step, assume that $\mathcal{X}_1 \subset \cdots \subset \mathcal{X}_h$, and let $\mathcal{X}_{h+1} = \mathcal{X}_h\cup \{j\}$ denote the best nested optimal $(h+1)$-portfolio, where $k = \argmax_{k \in \mathcal{C} \setminus \mathcal{X}_h }  \{ \Delta v(\mathcal{X}_h, k) \}$. We will show that $\mathcal{X}_{h+1}$ is globally optimal. To get a contradiction, suppose that $v(\mathcal{Y}_{h+1}) > v(\mathcal{X}_{h+1})$ for some other $(h+1)$ portfolio.
%
%Let $\mathcal{Y}_i$ denote the optimal $i$-portfolio comprised only of schools in $\mathcal{Y}_{h+1}$. Applying the inductive hypothesis, we have $\mathcal{Y}_1 \subset \cdots \subset \mathcal{Y}_{h+1}$. We can express the valuation of $\mathcal{Y}_{h+1}$ as follows.
%\begin{align}
%\mathcal{Y}_{h+1} &= 
%\end{align}
...
Let $\operatorname{Box}(x)$ denote the projection of the vector $x$ to the $1$-hypercube:
\begin{equation}\operatorname{Box}(x)_i = \max\bigl\{0, \min\{1, x_i \}\bigr\}, \end{equation}
...
\begin{equation}x = \operatorname{Box}\bigl[x + (f(x) - t)\bigr] \end{equation}
...
\For{$j= 1 \cdots J$}
{
    ...
    \For{$k= 1 \cdots K$}
    {
         ...
         \lIf{$ \bigl|x_i^{(k)} -x_i^{(k-1)} \bigr| \leq \epsilon, i = 1\dots n$}
         {
         	    \Return{$x^{(k)}$}
         }
    }
...
In all cases, $a, b, q$, and $n$ are \dots and we create 200 instances. 

We analyze the results graphically. T-tests show that convergence failures do not depend statistically on the experimental variables.

\begin{figure}[h!]
  \begin{center}\includegraphics[width=\textwidth]{./TwoSchoolModels/CombiPref-complementarity/heterogeneity-S.pdf}\end{center}
  \caption{(Heterogeneity experiment.) At times, we might want to put the caption on the side instead of at the top or bottom of the figure. The sidecap package can be used to place a caption next to the figure in such a case (it can also be used for other floating environments). }
\end{figure}

\begin{figure}[h!]
  \begin{center}\includegraphics[width=\textwidth]{./TwoSchoolModels/CombiPref-complementarity/heterogeneity-T.pdf}\end{center}
  \caption{(Heterogeneity experiment.) At times, we might want to put the caption on the side instead of at the top or bottom of the figure. The sidecap package can be used to place a caption next to the figure in such a case (it can also be used for other floating environments). }
\end{figure}

\begin{figure}[h!]
  \begin{center}\includegraphics[width=\textwidth]{./TwoSchoolModels/CombiPref-complementarity/heterogeneity-U.pdf}\end{center}
  \caption{(Heterogeneity experiment.) At times, we might want to put the caption on the side instead of at the top or bottom of the figure. The sidecap package can be used to place a caption next to the figure in such a case (it can also be used for other floating environments). }
\end{figure}

\begin{figure}[h!]
  \begin{center}\includegraphics[width=\textwidth]{./TwoSchoolModels/CombiPref-complementarity/heterogeneity-k.pdf}\end{center}
  \caption{(Heterogeneity experiment.) At times, we might want to put the caption on the side instead of at the top or bottom of the figure. The sidecap package can be used to place a caption next to the figure in such a case (it can also be used for other floating environments). }
\end{figure}

\begin{figure}[h!]
  \begin{center}\includegraphics[width=\textwidth]{./TwoSchoolModels/CombiPref-complementarity/complementarity-S.pdf}\end{center}
  \caption{(Complementarity experiment.) At times, we might want to put the caption on the side instead of at the top or bottom of the figure. The sidecap package can be used to place a caption next to the figure in such a case (it can also be used for other floating environments). }
\end{figure}

\begin{figure}[h!]
  \begin{center}\includegraphics[width=\textwidth]{./TwoSchoolModels/CombiPref-complementarity/complementarity-T.pdf}\end{center}
  \caption{(Complementarity experiment.) At times, we might want to put the caption on the side instead of at the top or bottom of the figure. The sidecap package can be used to place a caption next to the figure in such a case (it can also be used for other floating environments). }
\end{figure}

\begin{figure}[h!]
  \begin{center}\includegraphics[width=\textwidth]{./TwoSchoolModels/CombiPref-complementarity/complementarity-U.pdf}\end{center}
  \caption{(Complementarity experiment.) At times, we might want to put the caption on the side instead of at the top or bottom of the figure. The sidecap package can be used to place a caption next to the figure in such a case (it can also be used for other floating environments). }
\end{figure}

\begin{figure}[h!]
  \begin{center}\includegraphics[width=\textwidth]{./TwoSchoolModels/CombiPref-complementarity/complementarity-k.pdf}\end{center}
  \caption{(Complementarity experiment.) At times, we might want to put the caption on the side instead of at the top or bottom of the figure. The sidecap package can be used to place a caption next to the figure in such a case (it can also be used for other floating environments). }
\end{figure}
...
Othman, Abraham, Eric Budish, and Tuomas Sandholm. 2020. ``Finding Approximate Competitive Equilibria: Efficient and Fair
