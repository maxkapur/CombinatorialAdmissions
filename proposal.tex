...
%%Combining the transformations above, we have 
%%\begin{align}
%%\bar t_j = 
%%\begin{cases}
%%t_j (1 - f_k), \quad & j < k \\
...
%%\end{cases}
%%\end{align}
%%which is equivalent to incorporating the fact that $1 - f_k x_k = 1 - f_k$ into the utility values associated with the first $k$ terms of $v(x)$.
%
%The validity of this transformation implies that the optimal $(h+1)$-portfolio over $\mathcal{C}$ must include, as a subset, the optimal portfolio among the portfolios of size $h$ that include school $k$. But $\mathcal{X}_h$ is the globally optimal $h$-portfolio. Therefore, $\mathcal{X}_h \subset \mathcal{X}_{h+1}$, which completes the proof. re, the solution to \eqref{solveforoptimalxwithk}, which maximizes $v(\mathcal{X}_{h+1})$, is equivalent to the union of $k$ and the solution to \eqref{transformedelimck}, which gives the optimal $h$-portfolio over $\mathcal{C} \setminus k$.  
%
\end{proof}
