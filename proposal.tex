...
\begin{align}
v(\mathcal{X}) &= \sum_{j\in\{0\}\cup\mathcal{X}} t_j f_j  \prod_{\substack{j’ \in \mathcal{X}: \\ j' > j}} (1 - f_{j’}), \quad \text{or equivalently,} \label{closedformportfoliovaluationX}\\
\qquad v(x) &= t_0 \prod_{j=1}^m (1 - f_{j}) + \sum_{j=1}^m x_j t_j f_j \prod_{j’ = j+1}^m (1 - f_{j’} x_{j’}) \label{closedformportfoliovaluationx}
\end{align}
\end{proposition}
...
\begin{definition}[Na\"ive algorithm for Alma’s problem]
...
The transformation to $\tilde t$ does not change the order of the $t_j$-values. Therefore, the expression on the right is itself a portfolio valuation function. In the corresponding market, $t$ is replaced by $\tilde t$ and $\mathcal{C}$ is replaced by $\mathcal{C}\setminus\{k\}$. Now, we obtain $w(\mathcal{Y})$ through one more transformation: Define $\bar t_j = \tilde t_j - \tilde t_0$ so that $t_0 = 0$ and let
...
\end{cases}
\end{align}
It is easy to verify that this is the composition of the two transformations (from $t$ to $\tilde t$, and from $\tilde t$ to $\bar t$) given in the proof. Now, the \emph{next} school added must be the optimal singleton portfolio in the modified market. But the optimal singleton portfolio consists simply of the school with the highest value of $f_j \bar t_j$. Therefore, by storing the set of candidate schools in a heap ordered by $f_j t_j$ and updating these values at each iteration, we eliminate the need to compute $v(\mathcal{X})$ entirely.
 
The algorithm below requires a data structure $\operatorname{MaxFibHeap}()$, a Fibonacci heap whose entries are tuples $\mathtt{D} = (\mathtt{D.j},\mathtt{D.f}, \mathtt{D.t})$ of information about each college. The heap is ordered according to the criterion $\mathtt{D}\geq \mathtt{E} \iff \mathtt{D.f}\cdot \mathtt{D.t}\geq \mathtt{E.f}\cdot \mathtt{E.t}$. The algorithm outputs a list $\mathtt{X}$ of the $h$ schools to which Alma should apply. The schools appear in the order of entry such that when the algorithm is run with $h=m$, the optimal portfolio of size $h$ is given by $\mathcal{X}_h = \{\mathtt{X[1]}, \dots, \mathtt{X[h]}\}$. The entries of the list $\mathtt{V}$ give the valuation thereof.
...
$\mathtt{H} \gets \operatorname{MaxFibHeap}()$, an empty heap of named tuples $\mathtt{C}$ ordered by $\mathtt{C.f}\cdot\mathtt{C.t}$\;
\lFor{$j=1\dots m$}
...
Algorithm \ref{algorithmforlargeh} produces an optimal application portfolio in $O(h m + m \log m)$ time.
\end{theorem}
\begin{proof}
Optimality follows from the proof of Theorem \ref{nestedapplication}. The initialization steps are equivalent to sorting the schools by $t_j$ and $f_j t_j$, and therefore their computation time is $O(m \log m)$. At each of the $h$ iterations of the main loop, extracting the top school costs $O(\log m)$. There are $O(m)$ schools whose $t_j$-values are updated, and these updates cost $O(1)$ (amortized) on a Fibonacci heap (Fredman and Tarjan 1987). Therefore, the overall time complexity is $O(h m + m\log m)$.
\end{proof}
In fact, the time complexity remains $O(h m + m\log m)$ if a list is used instead of a heap. Rather than a list or Fibonacci heap, we found a simple binary heap to be the most effective data structure in our numerical experiments due to its low overhead cost. Although using a binary heap nominally increases the cost of updating to the $t_j$-values from $O(1)$ to $O(\log m)$, typical problem instances do not achieve this upper bound because the order of the $f_j t_j$-values changes only slightly between iterations. 
...
We will provide an algorithmic solution to Ellis's problem that runs in $O(Hm + m\log m)$ time and $O(m)$ space. The algorithm resembles common recursive algorithms for the binary knapsack problem. Because we cannot assume that $H \leq m$ (as was the case in Alma's problem), this represents a quasipolynomial-time solution. %Cite (\emph{Wikipedia}, s.v. "Knapsack problem") and Garey and Johnson
...
\begin{proposition}[Validity of Algorithm \ref{algorithmforhetg}]
Algorithm \ref{algorithmforlargeh} produces an optimal application portfolio in $O(H m + m \log m)$ time and $O(m)$ space.
\end{proposition}

The algorithm above is written in a top-down recursive form that computes only the values of $V[j, h]$ necessary to obtain $V[m, H]$. In the worst case, the number of entries computed is $O(Hm)$. Because $V[j, h]$ is evaluated lazily, it is possible that some entries are computed more than once; to prevent this, its entries can be stored in a lookup table as they are computed, which decreases the computation time but increases the storage requirement from $O(m)$ to $O(Hm)$. 
